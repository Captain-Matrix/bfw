...
static int request_handler(struct mg_connection *conn, enum mg_event ev) {
 ...
 char l4[10];
  ...
  struct tm *T;
  if (ev == MG_REQUEST) {
    mg_printf_data(conn, "<html><title>bfw</title><body><table border=\"1\">");
            mg_printf_data(conn,
    "<tr><td>%4s</td> <td>%3s</td> <td>%6s</td>  <td>%s</td>  <td>%15s</td> <td>%17s</td> <td>%15s</td> <td>%15s</td> <td>%3s</td> <td>%5s</td> <td>%5s</td> <td>%4s</td> <td>%8s</td> <td>%4s</td> <td>%5s</td>  <td>%7s</td></tr>\n<br/>",
      "#", "Hits","ACTION", "L3", "SOURCE", "ACL MASK", "DEST", "ACL MASK", "L4",
	...
	default:
      ...
      if(r->direction==EGRESS)snprintf(direction,10,"EGRESS");
      else if(r->direction==INGRESS)snprintf(direction,10,"INGRESS");
      
      mg_printf_data
	(conn, " <tr><td>%3d</td> <td>%4d</td>   "
	 " <td>%6s</td>  <td>%s</td>  <td>%15s</td> <td>%15s</td> <td>%15s</td>"
	 " <td>%15s</td> <td>%3s</td>  <td>%4d</td> <td>%5d</td> "
	 "<td>%4d KB</td> <td>%8s</td> <td>%2s</td><td>%2s</td> "
	 "<td>%5s</td>  <td>%7s</td></tr>\n",r->number,
	  r->hits,action, l3, int_to_ip (r->src),
	 ...
	 Dow, hour, minute, r->IF,direction);
      //r = CIRCLEQ_NEXT (r, entries);
        ...
        mg_printf_data(conn, "</table></body></html>");
